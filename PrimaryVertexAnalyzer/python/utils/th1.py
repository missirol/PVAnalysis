...
def get_histo_from_filepath(file_path, hpath, bins=[], auto_rebin=False):
    ...
    if not tfile: raise SystemExit(1)
...
def get_histo_from_rootfile(rootfile, hpath, bins=[], auto_rebin=False):
    if not rootfile:
      KILL('get_histo_from_rootfile -- input error: invalid input ROOT TFile object')
...
def get_histo_from_roottree(rootfile, bins=[], auto_rebin=False, **tree_dc):
  if not rootfile:
    KILL('get_histo_from_roottree -- input error: invalid input ROOT TFile object')
