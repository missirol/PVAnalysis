...
def get_efficiency_f(passed, total):
    hP1 = ROOT.TH1F('hP1', 'hP1', 1, 0, 1)
    hT1 = ROOT.TH1F('hT1', 'hT1', 1, 0, 1)
    hP1.SetBinContent(1, passed)
    hT1.SetBinContent(1, total)
    g = ROOT.TGraphAsymmErrors(hP1, hT1, 'cl=0.683 b(1,1) mode')
    if g.GetN() != 1:
      WARNING('get_efficiency_f -- failed efficiency computation: '+hP1.GetName()+' / '+hT1.GetName()+' [N='+str(g.GetN())+']')
      return [0., 0., 0.]
